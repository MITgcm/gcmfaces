...
for ff=1:ntile;
    ...
    if doCreate;
        ...
        for dd=1:length(dimlist);
            ...
            if strcmp(dimname{dd},'time');
              ncputAtt(ncid,dimlist{dd},'standard_name','time');
              ncputAtt(ncid,dimlist{dd},'units','days since 1992-1-1 0:0:0');
              if ~isempty(clmbnds);
                  ncputAtt(ncid,dimlist{dd},'climatology','climatology_bounds');
                  ncdefVar(ncid,'climatology_bounds',xtype,{'tcb' dimlist{dd}});
                  ncputAtt(ncid,'climatology_bounds','long_name','climatology_bounds');
                  ncputAtt(ncid,'climatology_bounds','units','days since 1992-1-1 0:0:0');
              end;
            elseif strcmp(dimname{dd},'longitude');
              ncputAtt(ncid,dimlist{dd},'standard_name','longitude');
              ncputAtt(ncid,dimlist{dd},'units','degrees_east');
            elseif strcmp(dimname{dd},'latitude');
              ncputAtt(ncid,dimlist{dd},'standard_name','latitude');
              ncputAtt(ncid,dimlist{dd},'units','degrees_north');
            elseif strcmp(dimname{dd},'depth');
              ncputAtt(ncid,dimlist{dd},'standard_name','depth');
              ncputAtt(ncid,dimlist{dd},'units','m');
              ncputAtt(ncid,dimlist{dd},'positive','down');
            else;
    ...
    if strcmp(fldName,'lon'); 
      ncputAtt(ncid,fldName,'standard_name','longitude');
      ncputAtt(ncid,fldName,'units','degrees_east');
    end;
    if strcmp(fldName,'lat');
      ncputAtt(ncid,fldName,'standard_name','latitude');
      ncputAtt(ncid,fldName,'units','degrees_north');
    end;
%   if strcmp(fldName,'dep_c')||strcmp(fldName,'dep_u')||strcmp(fldName,'dep_l');
%     ncputAtt(ncid,fldName,'standard_name','depth');
%     ncputAtt(ncid,fldName,'units','m');
